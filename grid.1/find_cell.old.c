...
// exit(0);	 
 ...
 if ((star[star_id].type==3 || star[star_id].type==5) && p[i].flag==0)
   {
     if (acos(p[i].kz)<M_PI-fabs(asin(c[NRCELLS-1].x/star[star_id].z))) 
     ...
     index=-1; 
       ...
       { 
	 theta=acos(p[i].kz);
	 beta=M_PI-asin((star[star_id].z/c[NRCELLS-1].x)*pow(1-p[i].kz*p[i].kz,0.5));
	 alpha=theta-beta;
	 phi=atan2(p[i].ky,p[i].kx); if  (phi<0) phi=phi+2.*M_PI; 

	 if (beta<0.5*M_PI){printf("beta<90deg\n"); exit(0);}


	 mfp=star[star_id].z*sin(alpha)/sin(beta);
	 ...
	 p[i].r=c[NRCELLS-1].x;
     //printf("%.4e %.4e %.4e %.4e %.4e\n", p[i].x/AU,  p[i].y/AU, p[i].z/AU, sqrt(p[i].x*p[i].x+p[i].y*p[i].y+p[i].z*p[i].z)/AU,p[i].r/AU);
