...
// read run parameters (uses info from the grid, so needs to be called once again) 

    read_params ();

  /* read opacity (in cgs units) */
...
printf("\n:: Doing DUST MIX ...\n");
printf("\n:: *** WARNING :: params.opa.h must be the same for both opacities **** \n\n");
 ...
 // read restart or add source info
...
printf("\n:: Including radiation sources...\n");
 ...
 if (MANUAL_STAR==1 && MANUAL_STAR_TYPE==0) 
	{
 ...
 if (MANUAL_STAR==1 && MANUAL_STAR_TYPE==3) 
   	{
    printf(":: Including manual star...\n");
   	nstar=nstar+1;
    star[nstar].type=3;
 ...
 if (nstar<0) 
  ...
  find_obs_fbins(&fbin_obs[0],fbin,&dlambda_obs[0], &fbin_obs_min[0],&fbin_obs_max[0]);
 ...
 /* Initialize interaction counter table  */
      ...
      if (star[s].type==1)    /** InterStellar Radiation Field **/ 
	{
	  ...
	  star[s].lum=(4*M_PI*pow(star[s].radius,2.)*M_PI*bisrf.int_total);
          star[s].temp=pow(M_PI*bisrf.int_total/sigma_CONST,0.25);
      ...
      else
	{
	  if (star[s].sedfile==1)    /** star with given SED **/ 
	    ...
	    {
            star[s].lum=(star[s].dilution*4*M_PI*pow(star[s].radius,2)*sigma_CONST*pow(star[s].temp,4));  
...
// find next_n (changes for new lum source)

#ifndef DUST_MIX

    find_cells_next_n(c, ncells, star[s].lum/star[s].npackets, &mean_op);

#else

    find_cells_next_n(c, ncells, star[s].lum/star[s].npackets,&mean_op, &mean_op2);
...
#ifdef INFO_OUTPUT
	  printf("doing photon %.0f out of %0.f (star type %d)\n",(double) (photon_i), (1.*star[s].npackets),star[s].type); 
#endif
...
#pragma omp critical (output)
	  {	
	    ...
	    place_into_bbins(p,i,&bbin,&total_bbin[0],&fbin_obs[0],&fbin_obs_min[0], &fbin_obs_max[0],r_boundary,star,s);      /***/	    
#else
	    ...
	    if (DIM==3 && ISODIM==3)  /***/ 
	      ...
	      find_fbin_isogrid_3D(p,i,dbin,&isogrid,&fbin,&fbin_obs[0], &fbin_obs_min[0], &fbin_obs_max[0],&total_fbin,star,s);
		...
		if (DIM==2 || ISODIM==2 ) 
		  ...
		  find_fbin_isogrid_2D_I (p,i,dbin,&isogrid,&fbin,&fbin_obs[0], &fbin_obs_min[0], &fbin_obs_max[0], &total_fbin,star,s);
		...
		/* just phi symmetry  */
		...
		else
		  {
		    if (DIM==4 || ISODIM==4) 
		      ...
		      find_fbin_isogrid_2D_II(p,i,dbin,&isogrid,&fbin,&fbin_obs[0], &fbin_obs_min[0], &fbin_obs_max[0], &total_fbin,star,s);
		  }
	      }
		...
		printf("star: %d --> DONE photon %.2e out of %.2e (%.1f per cent)\n", s, (double)(photon_i),(double)(star[s].npackets), (double)(photon_i) /(double)(star[s].npackets)*100);
		...
		record_interactions(&intercount[0],(double)(star[s].npackets), interactions,s,star[s].dint,photon_i);
                ...
                write_sed(&fbin,&bisrf,dbin,r_boundary,&sed_flag,star[s].type); 
		...
		init_fbins(&fbin,opacity); /* needs to be zero  when input from previous file */		
                ...
                make_isogrid_spherical_case(&bbin,&isogrid,&background[0],star[s].type);  /* ... transform to pixels */
                isomap_idl(&isogrid);
                null_isogrid_npackets(&isogrid);  	       
...
#ifdef DO_ISOMAP  
  	       find_isogrid_intensity(&isogrid,&lambda_obs[0],&background[0],dbin,&dlambda_obs[0],star,s,r_boundary,&iso_flag,star[s].type);
	       isomap_idl(&isogrid);	       
#ifdef STATISTICS 
