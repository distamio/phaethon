...
void maketree2(bodyptr btab, int nbody)
{
    double cpustart;
    bodyptr p;
    int i;
...
/*
 * MY-NEWTREE: reclaim cells in tree, prepare to build new one.
 */



/*
 * MY-MAKECELL: return pointer to free cell.
 */

local cellptr mymakecell(void)
{
    cellptr c;
    int i;
	...
	for (i = 0; i < NSUB; i++)              /* loop over subcells       */
	  {
	    real_ncell=real_ncell+1;            /* count real cells         */
	  }

	real_ncell=real_ncell-1; /* because i substract the cell i open */  
       
      }    
    else 
      {                                      /* else use existing cell   */
...
/*
 * MY-LOADBODY: descend tree and insert body p in appropriate place.
 */
 
local void myloadbody(bodyptr p)
{
    cellptr q, c;
    int qind, k,i;
    real qsize, dist2;
    vector distv;
    int myind,previous_index,index,depth;
    int nextcell[10000];             /* an array of what;s the id of next cell */
    /* printf("REAL_NCELL...  %d\n",real_ncell); */
	...
	if (Type(Subp(q)[qind]) == BODY)         /* if another body reached  */	  	  
	  {
	    if (cnum(Subp(q)[qind])>=10)
	      {
		index=2;
		previous_index=cid(Subp(q)[qind]);  
		/*   printf("previous_index %d\n",previous_index); */
		depth=depth+1;
		
		DOTPSUBV(dist2, distv, Pos(p), Pos(Subp(q)[qind]));	/* check positions differ   */ 
		 if (dist2 == 0.0) printf("loadbody: two bodies have same position\n"); 
		
		c = mymakecell();          /* then allocate new cell   */	    
		
		nextcell[previous_index]=real_ncell-6;
		
		for (i = 0; i < NSUB-1; i++)  nextcell[real_ncell-6+i]=real_ncell-6+i+1;
		...
		myind=subindex((bodyptr) Subp(q)[qind], c);
		
		/* printf("myind  %d\n",myind); */  /*previous body's qind */
		
		if (myind!=0) cid(Subp(q)[qind])=real_ncell-7+myind;
		else   cid(Subp(q)[qind])=previous_index;
		
		csize(Subp(q)[qind])=rsize/pow(2.,depth);
		
		Subp(c)[myind] = Subp(q)[qind]; /* put previous(?) body in cell  */	   
		
		/* printf("--OLD BODY CELL %d\n",cid(Subp(q)[qind]));  */
		
		Subp(q)[qind] = (nodeptr) c;        /* link cell in tree        */   /* previous body */	     
	      }
	    else
	      {index=4;break;}	   
	  }	
	...
	if (index==4) break;
	  ...
	  {
	    /* printf("aa previous index %d qind %d\n",previous_index,qind); */
	    
	    q = (cellptr) Subp(q)[qind];            /* advance to next level    */      /* this body */
	    qind = subindex(p, q);                  /* get index to examine     */
	    qsize = qsize / 2;                      /* shrink current cell      */  
	    
	    if (Subp(q)[qind] != NULL){ previous_index=nextcell[previous_index]+qind-1;
	    /* 	printf("bb previous index %d qind %d\n",previous_index,qind);*/ }
	    
	    depth=depth+1;
	  }
      }
    
    if (index==4) cnum(Subp(q)[qind])=cnum(Subp(q)[qind])+1;
    
    else
      {
	Subp(q)[qind] = (nodeptr) p;                /* found place, store p     */	     
	
	if (index==0)   cid(p)=qind+1;     /* this is for the new particle. i have to do this for all particles */
	
	if (index==1)
	  {
	    if (qind!=0) cid(p)=previous_index+qind; 
	    else cid(p)=nextcell[previous_index]+qind;
	  }
	
	if (index==2)
	  {
	    if (qind!=0) cid(p)=real_ncell-7+qind; 
	else cid(p)=previous_index; 
	  }
	
    
	cnum(p)=cnum(p)+1;
	csize(p)=rsize/pow(2.,depth);
	
	cindex(p)=index;
      }

    
    /* put particles at the center of the cell */

    if (qind==0)
      {
	Pos(p)[0]=Pos(q)[0]-csize(p)/2.; 
	Pos(p)[1]=Pos(q)[1]-csize(p)/2.;
	Pos(p)[2]=Pos(q)[2]-csize(p)/2.;
      }
   ...
   /* printf("--NEW BODY CELL %d  depth %d $$$$$$$$$$$$$$ INDEX%d\n",cid(p),depth,index );
      printf("--------------------------------------------------------------------");*/
} 
...
local void hackcofm(cellptr p, real psize, int lev)
{
    ...
    for (i = 0; i < NSUB; i++)                  /* loop over the subnodes   */
        if ((q = Subp(p)[i]) != NULL) {         /* skipping the NULLs       */
	     ...
	     if (Type(q) == BODY)  
	       {
		 csize(q)=csize(p)/2.; 
		 cid(q)=global_index+1; 
		 cnum(q)=1;
		 cdens(q)=Mass(q)/pow(csize(q),3.); 
    ...
    /* here i have made my changes */
    /* SETV(Pos(p), cmpos);*/                  /* and center-of-mass pos   */
  
    /*cnum(p)=Mass(p)/NSPH_MASS;  */ /* sph particles in the cell */
 
    cdens(p)=Mass(p)/pow(csize(p),3.);         
    cid(p)=global_index+1;
    global_index=global_index+1;

    /* printf ("mass:%.4e size:%.4e dens:%.4e\n",Mass(p), csize(p),cdens(p)); */
