/*
   ...
   compile: cc -o mopacity mopacity.c ../main/opacity.c ../main/inout.c ../main/bb.c -lm 
...
a_aux=(h_CONST/k_CONST)*100000*f2;
...
for (i=1;i<TEMPDIM;i++)
 ...
 /* i calculate the integral only were the BB function is important
   ...
   mean_op.opacity[i]=M_PI*
     (simp_kb(f1,8.e11*mean_op.temp[i],mean_op.temp[i]))   
