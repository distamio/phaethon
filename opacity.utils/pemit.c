/*
   ...
   compile: cc -g -o pemit pemit.c ../main/opacity.c  ../main/inout.c ../main/bb.c  -lm
...
/*** find max of the probabity distribution  */   
...
for(k=1;k<TEMPDIM;k++)
  ...
  reemit.rfreq[k]=reemit.freq[0];

   for (i=1;i<NFREQ;i++)

        if ( k_abs(reemit.freq[i],opacity)*dblack(reemit.freq[i],temp)> 
	     k_abs(reemit.freq[k],opacity)*dblack(reemit.freq[k],temp) )  
            {
              reemit.fpoint[k]=i; 
              reemit.rfreq[k]=reemit.freq[i];   
             }
}
 
 fp = fopen("rtable.dat", "w");
 for (i=1;i<TEMPDIM; i++)
   {
     fprintf(fp,"%.2e %d\n", reemit.rfreq[i],reemit.fpoint[i]);
   }
 fclose(fp);
